...
**Release: Arduino meter stability and data-integrity fixes**

### Added
- Health status topic (`arduino/1/health`) for Home Assistant monitoring.
- Meter-value delta validation and self-healing diagnostics.

### Fixed
- ATmega16U2 USB bridge lockups by removing Serial0 debug flooding.
- Energy-meter cross-contamination after reboots by discarding initial telegrams.
- Heap pressure in serial parsing by eliminating String allocation from PowerSerial.
- Invalid six-hour preventive reset that could wedge the USB bridge.

### Security
- Generated `compile_commands.json` is no longer tracked and is ignored because it can contain private build flags.
