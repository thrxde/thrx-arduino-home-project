...
char topicHealth[]     = "arduino/1/health";
...
void loop() {
	...
	if (millis() - lastStatusPublish >= 5000) {
		...
		// Publish sensor health — rejection counts indicate bit-flip contamination
		uint8_t swuBR = PowerSerial::swu.getRejectCountBezug();
		uint8_t swuLR = PowerSerial::swu.getRejectCountLiefer();
		uint8_t solBR = PowerSerial::solar.getRejectCountBezug();
		uint8_t solLR = PowerSerial::solar.getRejectCountLiefer();
		if (swuBR > 0 || swuLR > 0 || solBR > 0 || solLR > 0) {
			char healthBuf[32];
			snprintf(healthBuf, sizeof(healthBuf), "WARN:%u/%u/%u/%u",
				swuBR, swuLR, solBR, solLR);
			mqttClient.publish(topicHealth, healthBuf);
		} else {
			mqttClient.publish(topicHealth, "OK");
		}
